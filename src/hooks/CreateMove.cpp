...
static settings::Boolean forward_speedhack_pitch_down{ "misc.forward-speedhack-pitch-down", "false" };
static settings::Boolean forward_speedhack_pitch_up{ "misc.forward-speedhack-pitch-up", "false" };
    ...
    if (CE_GOOD(g_pLocalPlayer->entity))
    {
        ...
        if (roll_speedhack && cmd->buttons & IN_DUCK && (CE_INT(g_pLocalPlayer->entity, netvar.iFlags) & FL_ONGROUND) && !(cmd->buttons & IN_ATTACK) && !HasCondition<TFCond_Charging>(LOCAL_E))
        {
            ...
            if (fabs(speed) > 0.0f)
            {
                ...
                if (forward_speedhack_pitch_down)
                {
                    cmd->forwardmove *= -1.0f;
                    cmd->sidemove *= -1.0f;
                    cmd->viewangles.x = 91;
                }
                if (forward_speedhack_pitch_up)
                {
                    cmd->forwardmove *= -1.0f;
                    cmd->sidemove *= -1.0f;
                    cmd->viewangles.x = -91;
